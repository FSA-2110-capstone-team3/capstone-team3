...
const Comments = ({ episodeId, episodeSpotifyId }) => {
  ...
  const commentLikes =
    useSelector((state) =>
      state.commentLikes.filter((commentLike) => commentLike.spotify_id === id)
    ) || [];
    ...
    <div>
      ...
      {epComments
        ...
        .map((comment) => {
          ...
          const matchingCmtLike =
            commentLikes.find(
              (cmtLike) =>
                cmtLike.commentId === comment.id && cmtLike.userId === auth.id
            ) || {};
          const allCommentLikes =
            commentLikes.filter(
              (cmtLike) =>
                cmtLike.thumbsUp === true && cmtLike.commentId === comment.id
            ) || [];
          const allCommentDislikes =
            commentLikes.filter(
              (cmtLike) =>
                cmtLike.thumbsDown === true && cmtLike.commentId === comment.id
            ) || [];
          ...
          const isEditedCmt = comment.edited ? (
            <Tooltip>
              <div style={{ color: "lightgrey", marginLeft: "10px" }}>
                edited
              </div>
            </Tooltip>
          ) : (
              ...
              >
                ...
                {repliesOpen.isLoaded && comment.id === repliesOpen.id ? (
                  <>
                    {commentReplies.length > 0 ? (
                      <p
                        ...
                        onClick={() =>
                          setRepliesOpen({ isLoaded: false, id: "" })
                        }
                      >
                        Hide Replies
                      </p>
                    ...
                    {commentReplies.map((reply) => {
                      const replyUser =
                        ...
                        {};
                      const matchingReplyLike =
                        commentLikes.find(
                          (cmtLike) =>
                            cmtLike.commentId === reply.id &&
                            cmtLike.userId === auth.id
                        ) || {};
                      const allReplyLikes =
                        commentLikes.filter(
                          (cmtLike) =>
                            cmtLike.thumbsUp === true &&
                            cmtLike.commentId === reply.id
                        ) || [];
                      const allReplyDislikes =
                        commentLikes.filter(
                          (cmtLike) =>
                            cmtLike.thumbsDown === true &&
                            cmtLike.commentId === reply.id
                        ) || [];
                      ...
                      const isEditedReply = reply.edited ? (
                        <Tooltip>
                          <div
                            style={{ color: "lightgrey", marginLeft: "10px" }}
                          >
                            edited
                          </div>
                        </Tooltip>
                            ...
                            onClick={() => {
                              if (matchingReplyLike.commentId === reply.id) {
                                if (matchingReplyLike.thumbsDown) {
                                  dispatch(
                                    deleteCommentLike(matchingReplyLike.id)
                                  );
                                  dispatch(
                                    addCommentLike({
                                      spotify_id: id,
                      ...
                      return (
                        <>
                          <Comment
                            datetime={
                              <Tooltip
                                title={dayjs(reply.updatedAt).format("L LT")}
                              >
                  ...
                  <p
                    style={{ cursor: "pointer", fontSize: "14px" }}
                    onClick={() =>
                      setRepliesOpen({ isLoaded: true, id: comment.id })
                    }
                  >
                    Load {commentReplies.length} replies
                  </p>
                ) : (
                  ""
                )}
