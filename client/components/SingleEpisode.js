...
import { addComment, getSingleEpisode, addTimeStamp, getTimeStamps } from '../store';
...
const SingleEpisode = () => {
  ...
  const timeStamps = useSelector((state) => state.timeStamps.filter((timeStamp) => timeStamp.spotify_id === id)) || [];
  ...
  const hourLength = episode.duration_ms ? Math.floor(episode.duration_ms / 3600000) + 1 : 0;
    ...
    <div style={{'display': 'flex', 'flexDirection': 'column', 'marginLeft': '10px'}}>
      ...
      <div>
        <div style={{'color': 'white', 'width': '100%'}}>
          ...
          <form onSubmit={submitTimeStamp}>
            <div>
              ...
              <select
                ...
                name='hr'
              >
                <option value={'Select Hr'} disabled>Select Hr</option>
                {
                  Array(hourLength).fill('').map((min, idx) => {
                    return (
            ...
            <div>
              ...
              <select 
                ...
                name='min'
              >
                <option value={'Select Min'} disabled>Select Min</option>
                {
                  Array(60).fill('').map((min, idx) => {
                    return (
                      <option value={idx}>{idx}</option>
                    )
                  })
            ...
            <div>
              ...
              <select
                ...
                name='sec'
              >
                <option value={'Select Sec'} disabled>Select Sec</option>
                {
                  Array(60).fill('').map((sec, idx) => {
                    return (
                      <option value={idx}>{idx}</option>
                    )
                  })
          ...
          {
            !timeStamps.length ?
              <h5 style={{ color: "white" }}>No current Timestamps! Create one now!</h5> :
              timeStamps.sort((a,b) => a.timeStamp - b.timeStamp).map((timeStamp) => {
                return (
                  <div style={{'color': 'white', 'width': '70%'}} key={timeStamp.id}>
                    <span style={{'color': 'orange', 'cursor': 'pointer'}} onClick={() => setStamp(convertToSec(timeStamp.hr, timeStamp.min, timeStamp.sec))}>
                      {timeStamp.hr}:{timeStamp.min < 10 ? '0' + timeStamp.min : timeStamp.min}:{timeStamp.sec < 10 ? '0' + timeStamp.sec : timeStamp.sec}
      ...
      <div>
        <div>
        <h3 style={{'color': 'white', 'width': '70%'}}>COMMENTS ({epComments.length})</h3>
