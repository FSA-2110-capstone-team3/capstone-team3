...
router.post("/spotify/episodes", async (req, res, next) => {
  try {
    ...
    const access_token = curr_user.access_token;
    const response = (
      await axios.get("https://api.spotify.com/v1/me/episodes", {
        form: {
          market: "US",
        },
        headers: {
...
router.post("/spotify/remove/:id", async (req, res, next) => {
  try {
    ...
    const access_token = curr_user.access_token;
    const response = (
      await axios.delete("https://api.spotify.com/v1/me/episodes", {
        params: {
          ids: `${episodeId}`,
        },
        form: {
...
router.get("/spotify/shows/contains", async (req, res, next) => {
  try {
    ...
    const access_token = curr_user.access_token;
    const response = (
      await axios.get("https://api.spotify.com/v1/me/shows/contains", {
        params: {
          ids: shows,
        },
        headers: {
...
router.get("/spotify/episodes/contains", async (req, res, next) => {
  try {
    ...
    const access_token = curr_user.access_token;
    const response = (
      await axios.get("https://api.spotify.com/v1/me/episodes/contains", {
        params: {
          ids: episodes,
        },
        headers: {
